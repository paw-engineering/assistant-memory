...
type IndexFlatL2 = InstanceType<typeof IndexFlatL2>;
...
export class FaissIndex {
  ...
  load(): void {
    ...
    const loaded = JSON.parse(readFileSync(this.idMapPath, "utf-8")) as string[];
    this.ids = loaded;
  }
   ...
   */
  rebuild(items: Array<{ id: string; embedding: number[] }>): void {
    this.reset();
    for (const item of items) {
      this.add(item.id, item.embedding);
    }
